...
const EnterTeamPage = () => {
  ...
  useEffect(() => {
    ...
    api.get('/config/enter_team_deadlines').then(r => {
      if (r.data?.value) {
        try {
          const parsed = typeof r.data.value === 'string' ? JSON.parse(r.data.value) : r.data.value;
          if (Array.isArray(parsed)) setDeadlines(parsed);
        } catch (e) { console.error("Failed to parse deadlines", e); }
      }
    }).catch(() => { });
    ...
    api.get('/config/enter_team_fees').then(r => {
      if (r.data?.value) {
        try {
          const parsed = typeof r.data.value === 'string' ? JSON.parse(r.data.value) : r.data.value;
          if (Array.isArray(parsed)) setFees(parsed);
        } catch (e) { console.error("Failed to parse fees", e); }
      }
    }).catch(() => { });
  }, []);
