...
const AdminRules = () => {
    ...
    const fetchRules = () => { api.get('/rules').then(r => setRules(r.data || [])).catch(() => toast.error('Failed to fetch rules')); };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editId) { await api.put(`/rules/${editId}`, data); toast.success('Rule section updated'); }
            else { await api.post('/rules', data); toast.success('Rule section created'); }
    ...
    const handleDelete = async (id: string) => {
        ...
        try { await api.del(`/rules/${id}`); fetchRules(); toast.success('Rule section deleted'); } catch (err: any) { toast.error(err.message); }
    };
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                    ...
                    <div className="lg:col-span-1 order-1 lg:order-2">
                        <div className="bg-gray-800 border border-white/10 p-4 sticky top-4">
                            ...
                            />
                            <p className="text-[10px] text-gray-600 mt-2 text-center">Ref: rules-list.webp</p>
                        </div>
                    </div>
                </div>
            )}
