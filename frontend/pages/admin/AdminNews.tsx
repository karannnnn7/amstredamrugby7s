...
const AdminNews = () => {
    ...
    const fetchNews = () => { api.get('/news').then(r => setNews(r.data || [])).catch(() => { }); };
    useEffect(() => { fetchNews(); }, []);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editId) { await api.upload(`/news/${editId}`, fd, 'PUT'); }
            else { await api.upload('/news', fd); }
            fetchNews(); resetForm();
    ...
    const handleDelete = async (id: string) => {
        ...
        try { await api.del(`/news/${id}`); fetchNews(); } catch (err: any) { alert(err.message); }
    };
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                    ...
                    <div className="lg:col-span-1 order-1 lg:order-2">
                        <div className="bg-gray-800 border border-white/10 p-4 sticky top-4">
                            ...
                            <img
                                src="/assets/admin-previews/news.png"
                            ...
                            />
                            <p className="text-[10px] text-gray-600 mt-2 text-center">Ref: news.png</p>
                        </div>
                    </div>
                </div>
            )}
