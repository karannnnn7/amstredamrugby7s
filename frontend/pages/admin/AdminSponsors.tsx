...
const AdminSponsors = () => {
    ...
    const fetchSponsors = () => { api.get('/sponsors').then(r => setSponsors(r.data || [])).catch(() => { }); };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editId) { await api.upload(`/sponsors/${editId}`, fd, 'PUT'); }
            else { await api.upload('/sponsors', fd); }
    ...
    const handleDelete = async (id: string) => {
        ...
        try { await api.del(`/sponsors/${id}`); fetchSponsors(); } catch (err: any) { alert(err.message); }
    };
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                    ...
                    <div className="lg:col-span-1 order-1 lg:order-2">
                        <div className="bg-gray-800 border border-white/10 p-4 sticky top-4">
                            ...
                            <img
                                src="/assets/admin-previews/sponsors-page.png"
                                onError={(e) => (e.currentTarget.style.display = 'none')}
                                alt="Sponsors Preview"
                                className="w-full h-auto object-cover border border-white/5"
                            />
                            <p className="text-[10px] text-gray-600 mt-2 text-center">Ref: sponsors-page.png</p>
                        </div>
                    </div>
                </div>
            )}
